<template>
	<view class="sort-view">
		...
		<view class="sort-right">
			<view class="Commodity" v-for="(item,index) in gifts" :key="index">
...
<script setup>
	...
	const {sort,gifts,num} = toRefs(data)
	...
	async function gooDs(){
		...
		// 请求商品数据
		const res_goods = await DB.database().collection('gifts').where({category:res_sort.data[0]?.sort_name}).limit(10).field(field_obj).get()
	...
	onReachBottom(async()=>{
		...
		let DB = await inIt()
		const res_goods = await DB.database().collection('gifts').where({category:data?.sort_name}).limit(10).skip(sk).field(field_obj).get()
